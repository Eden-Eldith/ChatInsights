...
class ChatInsightsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ChatInsights v2 - AI Chat Analysis Tool (ChatGPT & Claude)")
    ...
    def create_import_tab(self):
        ...
        # File selection
        file_frame = ttk.LabelFrame(frame, text="AI Chat Export File (ChatGPT or Claude)")
    ...
    def detect_platform(self, data):
        """Detect whether the JSON is from ChatGPT or Claude"""
        if isinstance(data, list):
            # Check if it's ChatGPT format (list of conversations)
            if len(data) > 0 and isinstance(data[0], dict):
                ...
                if 'uuid' in data[0]:
                    ...
                    if 'chat_messages' in data[0] or 'name' in data[0]:
                        return "claude"
        elif isinstance(data, dict):
            # Sometimes exports might be wrapped in an object
        ...
        try:
            if isinstance(data, list) and len(data) > 0:
                first_item = data[0]
                # Check for Claude's chat_messages structure
                if 'chat_messages' in first_item and isinstance(first_item['chat_messages'], list):
                    return "claude"
                # Fallback check for other Claude indicators
...
Works with both ChatGPT and Claude conversation exports!
...
4. Generate a knowledge graph of your AI interactions
        ...
        default_concepts = """
AI: \\bAI\\b|Artificial Intelligence|GPT|Claude|LLM|Language Model
...
Now supports both ChatGPT and Claude conversation exports!
...
Features:
- Auto-detection of ChatGPT vs Claude exports
...
- Training data generation from multiple AI assistants
    ...
    def get_claude_messages(self, conversation):
        """Get messages from a Claude conversation"""
        messages = []
